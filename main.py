...
from PnPFISTANet import PnPFISTANet, ExternalDenoiser
import argparse
...
# Evaluate reconstructed images with PSNR, SSIM, RMSE.
...
denoiser = ExternalDenoiser().to(device)
fista_net = PnPFISTANet(LayerNo=7, Phi=J_eit, L=DMts, mask=mask, denoiser=denoiser)
...
parser = argparse.ArgumentParser()
parser.add_argument('--model_name', type=str, default='PnPFISTANet')
parser.add_argument('--num_epochs', type=int, default=5)
parser.add_argument('--lr', type=float, default=1e-3)
parser.add_argument('--data_dir', type=str, default=data_dir)
parser.add_argument('--save_path', type=str, default='./models/PnPFISTANet/')
parser.add_argument('--start_epoch', type=int, default=0)
parser.add_argument('--multi_gpu', type=bool, default=False)
parser.add_argument('--device', default=device)
parser.add_argument('--log_interval', type=int, default=20)
...
pnpfista_net_test = fista_net_test.cpu().double()
fig_name = dir_name + '/pnpfista_net_' + str(args.test_epoch) + 'epoch.png'
results = [test_images, x_lap, FBPConv_test, ISTANet_test, pnpfista_net_test]
# Evaluate reconstructed images with PSNR, SSIM, RMSE.
p_fista, s_fista, m_fista = compute_measure(test_images, pnpfista_net_test, 1)
print('PSNR: {:.5f}\t SSIM: {:.5f} \t RMSE: {:.5f}'.format(p_fista, s_fista, m_fista))
titles = ['truth', 'Lap. Reg.', 'FBPConv', 'ISTANet', 'pnpfista_net']
show_image_matrix(fig_name, results, titles=titles, indices=slice(0, 15))